 ...
 */
public class KafkaServerProviderImpl extends KafkaServerProvider {

	protected static final Logger LOG = LoggerFactory.getLogger(KafkaServerProviderImpl.class);
	...
	public void prepare(int numKafkaServers) {
		...
		try {
			...
			for (int i = 0; i < numKafkaServers; i++) {
				brokers.add(getKafkaServer(i, tmpKafkaDirs.get(i), KafkaServerProvider.KAFKA_HOST, zookeeperConnectionString));

				SocketServer socketServer = brokers.get(i).socketServer();
				brokerConnectionString += hostAndPortToUrlString(KafkaServerProvider.KAFKA_HOST, brokers.get(i).socketServer().boundPort(SecurityProtocol.PLAINTEXT)) + ",";
			}
