...
/**
 * This class contains tests shared by all kafka versions
 */
...
public abstract class KafkaConsumerTestBase extends KafkaTestBase {
	...
	/**
	 * Find a leader of the specified topic
	 * @param topic topic
	 * @return tuple2 with broker id and hostname of leader
	 * @throws Exception
	 */
	protected abstract Tuple2<Integer, String> getLeader(String topic) throws Exception;
	...
	public void runSimpleConcurrentProducerConsumerTopology() throws Exception {
		...
		FlinkKafkaConsumerBase<Tuple2<Long, String>> source = getConsumer(topic, sourceSchema, standardProps);
	...
	public void runBrokerFailureTest() throws Exception {
		...
		Tuple2<Integer, String> leadertoShutDown = getLeader(topic);


		LOG.info("Leader to shutdown {}", leadertoShutDown.f0);
		...
		env
				.addSource(kafkaSource)
				.map(new PartitionValidatingMapper(parallelism, 1))
				.map(new BrokerKillingMapper<Integer>(leadertoShutDown.f1, failAfterElements, brokers))
				.addSink(new ValidatingExactlyOnceSink(totalElements)).setParallelism(1);
		...
		// start a new broker:
		brokers.set(leadertoShutDown.f0, getKafkaServer(leadertoShutDown.f0, tmpKafkaDirs.get(leadertoShutDown.f0), kafkaHost, zookeeperConnectionString));
	...
	private void writeSequence(StreamExecutionEnvironment env, String topicName, final int numElements, int parallelism) throws Exception {
	...
	private void printTopic(String topicName, int elements,DeserializationSchema<?> deserializer)
			throws IOException
	{
	...
	public static class BrokerKillingMapper<T> extends RichMapFunction<T,T>
		...
		public BrokerKillingMapper(String leaderToShutDown, int failCount, List<KafkaServer> brokers) {
