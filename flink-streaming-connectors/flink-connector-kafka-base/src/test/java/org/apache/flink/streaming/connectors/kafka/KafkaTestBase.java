...
public abstract class KafkaTestBase extends TestLogger {
	...
	protected List<KafkaServer> brokers;
	protected String brokerConnectionStrings = "";

	protected ConsumerConfig standardCC;
	protected Properties standardProps;
	
	protected ForkableFlinkMiniCluster flink;

	protected int flinkPort;

	protected FiniteDuration timeout = new FiniteDuration(10, TimeUnit.SECONDS);

	protected List<File> tmpKafkaDirs;

	protected String kafkaHost = "localhost";

	// ------------------------------------------------------------------------
	//  Setup and teardown of the mini clusters
	// ------------------------------------------------------------------------

	// initialize the test servers once
	@Rule
	public ExternalResource resource = new ExternalResource() {
		...
		protected void before() throws Throwable {
			...
			try {
				...
				for (int i = 0; i < NUMBER_OF_KAFKA_SERVERS; i++) {
					...
					HostAndPort hostPort = getHostPort(socketServer);
					brokerConnectionStrings += hostPort.toString() + ",";
				}
		...
		protected void after() {
			LOG.info("-------------------------------------------------------------------------");
			LOG.info("    Shut down test servers ");
			LOG.info("-------------------------------------------------------------------------");
	...
	protected abstract HostAndPort getHostPort(SocketServer socketServer);
	...
	protected KafkaServer getKafkaServer(int brokerId, File tmpFolder,
		...
		for (int i = 1; i <= numTries; i++) { 
			...
			try {
				KafkaServer server = createKafkaServer(kafkaConfig);
				server.startup();
				return server;
			}
	...
	abstract KafkaServer createKafkaServer(KafkaConfig kafkaConfig);
	...
	abstract void createTestTopic(String topic, int numberOfPartitions, int replicationFactor);

	abstract void deleteTestTopic(String topic);

	/*protected void createTestTopic(String topic, int numberOfPartitions, int replicationFactor) {
		

	}
	
	protected void deleteTestTopic(String topic) {
