...
public abstract class FlinkKafkaConsumerBase<T> extends RichParallelSourceFunction<T>
	...
	public enum OffsetStore {
		 ...
		 */
		FLINK_ZOOKEEPER("org.apache.flink.streaming.connectors.kafka.internals.ZookeeperOffsetHandler"),
	...
	public enum FetcherType {
		 ...
		 */
		LEGACY_LOW_LEVEL("org.apache.flink.streaming.connectors.kafka.internals.LegacyFetcher"),
		 ...
		 */
		NEW_HIGH_LEVEL("kafka09");

		private final String clazz;

		FetcherType(String clazz) {
			this.clazz = clazz;
		}
	 ...
	 */
	public FlinkKafkaConsumerBase(String topic, DeserializationSchema<T> deserializer, Properties props,
	 ...
	 *           The type of fetcher to use (new high-level API, old low-level API).
	 */
	public FlinkKafkaConsumerBase(String topic, KeyedDeserializationSchema<T> deserializer, Properties props,
								OffsetStore offsetStore, FetcherType fetcherType) {
	...
	protected abstract List<PartitionInfo> getPartitionsForTopic(String topic, Properties props);
	...
	public void open(Configuration parameters) throws Exception {
		...
		// create fetcher
		Class<?> fetcherClass = Class.forName(fetcherType.getClazz());
		Constructor<?> fetcherCtor = fetcherClass.getConstructor(String.class, Properties.class, String.class);
		fetcher = (Fetcher) fetcherCtor.newInstance(topic, props, getRuntimeContext().getTaskName());

		fetcher.setPartitionsToRead(subscribedPartitions);

		// create offset handler
		switch (offsetStore){
			case FLINK_ZOOKEEPER:
				Class<?> storeClass = Class.forName(offsetStore.getClazz());
				Constructor<?> storeCtor = storeClass.getConstructor(Properties.class);
				offsetHandler = (OffsetHandler) storeCtor.newInstance(props);
				break;
			case KAFKA:
	...
	private static class PeriodicOffsetCommitter extends Thread {
		...
		private final FlinkKafkaConsumerBase consumer;
		private volatile boolean running = true;

		public PeriodicOffsetCommitter(long commitInterval, FlinkKafkaConsumerBase consumer) {
