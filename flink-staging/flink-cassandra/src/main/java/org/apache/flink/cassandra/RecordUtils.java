...
/**
 * Utilities for converting Java data types into Cassandra data types.
 */
public final class RecordUtils implements Serializable {

	private static final Map<Class<?>, TypeSerializer<?>> serializers;

	static {
		serializers = new HashMap<Class<?>, TypeSerializer<?>>();
	...
	public static ByteBuffer toByteBuffer(final Object value) {
		...
		throw new RuntimeException(
				"Can not transform field (class: " + value.getClass() + ") to ByteBuffer");
	}
	...
	private static TypeSerializer getSerializer(Class<?> collectionType, Class<?> clazz) {
		if (!serializers.containsKey(clazz)) {
			throw new RuntimeException("Can not transform " + collectionType + " with element types of "
					+ clazz + " to ByteBuffer");
		}
