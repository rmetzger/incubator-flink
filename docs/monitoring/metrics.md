...
At the `latencyTrackingInterval`, the sources will periodically emit a special record, called a `LatencyMaker`.
The marker contains a timestamp from the time when the record has been emitted at the sources.
Latency marker can not overtake regular user records, thus if records are queuing up in front of an operator, 
